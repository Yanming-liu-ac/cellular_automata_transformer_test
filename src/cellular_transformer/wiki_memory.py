...
@dataclass(frozen=True)
class CAWikiCellMetadataImportanceLUTEntry:
    """Low-bit metadata bucket mapped to a page-importance mode."""
...
@dataclass(frozen=True)
class CAWikiCellMetadataImportancePoint:
    """Evaluation point for metadata-derived provenance importance."""
    ...
    estimated_policy_lut_bytes: float
    target_met: bool


@dataclass(frozen=True)
class CAWikiCellMetadataImportanceResult:
    """Metadata -> importance -> provenance repair controller diagnostic."""
    ...
    points: Tuple[CAWikiCellMetadataImportancePoint, ...]
...
@dataclass(frozen=True)
class CAWikiCellTextTraceImportancePoint:
    """Evaluation point for text-source compiled-wiki importance."""
...
@dataclass(frozen=True)
class CAWikiCellTextTraceImportanceResult:
    """Text-source compiled wiki trace -> provenance importance."""
    ...
    importance_modes: Tuple[str, ...]
    entries: Tuple[CAWikiCellCompiledTraceImportanceLUTEntry, ...]
    points: Tuple[CAWikiCellTextTraceImportancePoint, ...]


@dataclass(frozen=True)
class CAWikiCellTextTraceNoisePoint:
    """Evaluation point for parser-noisy text-source importance."""
    ...
    strict_rate: float
    mean_source_text_edits_per_claim: float
    mean_clean_retrieval_errors_per_claim: float
    mean_observed_retrieval_errors_per_claim: float
...
@dataclass(frozen=True)
class CAWikiCellTextTraceNoiseResult:
    """Parser-noisy text-source trace -> provenance importance."""
    ...
    importance_modes: Tuple[str, ...]
    entries: Tuple[CAWikiCellCompiledTraceImportanceLUTEntry, ...]
    points: Tuple[CAWikiCellTextTraceNoisePoint, ...]
...
@dataclass(frozen=True)
class CAWikiCellParagraphTracePoint:
    """Evaluation point for paragraph-style source/wiki traces."""
...
@dataclass(frozen=True)
class CAWikiCellParagraphTraceResult:
    """Paragraph-style source/wiki trace -> provenance importance."""
    ...
    importance_modes: Tuple[str, ...]
    entries: Tuple[CAWikiCellMultiFieldTraceLUTEntry, ...]
    points: Tuple[CAWikiCellParagraphTracePoint, ...]
...
@dataclass(frozen=True)
class CAWikiCellParagraphSplitConfidencePoint:
    """Evaluation point for split paragraph confidence controller variants."""
    ...
    normal_rate: float
    strict_rate: float
    mean_summary_core_gap_per_claim: float
    mean_source_core_gap_per_claim: float
    mean_agreement_gap_per_claim: float
    estimated_touch_per_event: float
    downgraded_strict_claims: int
    target_met: bool
...
@dataclass(frozen=True)
class CAWikiCellParagraphSplitConfidenceResult:
    """Split summary/source/agreement confidence controller tradeoff."""
...
def _ca_wiki_metadata_features(
    claim_count: int,
    seed: int,
) -> np.ndarray:
    rng = np.random.default_rng(seed)
    trust = rng.choice(np.array([0, 1, 2, 3]), size=claim_count, p=[0.20, 0.35, 0.30, 0.15])
    citations = rng.choice(
        np.array([0, 1, 2, 3]),
        size=claim_count,
        p=[0.30, 0.35, 0.25, 0.10],
    )
    recency = rng.choice(np.array([0, 1, 2, 3]), size=claim_count, p=[0.25, 0.35, 0.25, 0.15])
    queries = rng.choice(np.array([0, 1, 2, 3]), size=claim_count, p=[0.45, 0.30, 0.18, 0.07])
    return np.stack((trust, citations, recency, queries), axis=1).astype(np.int64)


def _ca_wiki_metadata_teacher(features: np.ndarray) -> np.ndarray:
    labels = np.zeros(features.shape[0], dtype=np.int64)
    trust = features[:, 0]
    citations = features[:, 1]
    recency = features[:, 2]
    queries = features[:, 3]
    score = 2 * trust + citations + recency + queries
    strict = ((trust >= 3) & ((citations >= 2) | (queries >= 2))) | (score >= 9)
    normal = (~strict) & ((score >= 6) | (queries >= 3) | ((recency >= 3) & (citations >= 2)))
    labels[normal] = 1
    labels[strict] = 2
    return labels
...
def _ca_wiki_source_status_text(claim: int, source: int, value: int, revision: int) -> str:
    return (
        ...
        f"status={_CA_WIKI_STATUS_TERMS[value]}; revision={revision}"
    )


def _ca_wiki_summary_status_text(claim: int, value: int, revision: int) -> str:
    return (
        f"- claim-{claim:04d}: status is {_CA_WIKI_STATUS_TERMS[value]} "
        f"(compiled rev {revision})"
    )


def _ca_wiki_parse_status_text(text: str) -> int:
    for index, term in enumerate(_CA_WIKI_STATUS_TERMS):
        if f"status={term}" in text or f"status is {term}" in text:
            return index
    return -1


def _ca_wiki_parse_status_text_noisy(
    text: str,
    rng: np.random.Generator,
    parser_misread_rate: float,
    parser_drop_rate: float,
) -> Tuple[int, int]:
    clean_value = _ca_wiki_parse_status_text(text)
    if clean_value < 0:
        return -1, 1
    if rng.random() < parser_drop_rate:
        return -1, 1
    if rng.random() < parser_misread_rate:
        alternatives = [
            value
            for value in range(len(_CA_WIKI_STATUS_TERMS))
...
def _ca_wiki_multifield_weighted_missing(values: np.ndarray) -> float:
    ...
    weights = _CA_WIKI_MULTI_FIELD_WEIGHTS[: missing.shape[-1]]
    if missing.ndim == 1:
        return float(np.sum(weights[missing]))
    return float(np.sum(missing.astype(np.float64) * weights))


def _ca_wiki_multifield_core_agreement_gap(
    summary: np.ndarray,
    source_values: np.ndarray,
) -> float:
    gap = 0.0
    for field_index in range(2):
        present = source_values[:, field_index][source_values[:, field_index] >= 0]
        summary_value = int(summary[field_index])
        if len(present) < 2:
            gap += float(_CA_WIKI_MULTI_FIELD_WEIGHTS[field_index])
        elif len(set(int(value) for value in present)) > 1:
            gap += float(_CA_WIKI_MULTI_FIELD_WEIGHTS[field_index])
        elif summary_value >= 0 and summary_value != int(present[0]):
            gap += float(_CA_WIKI_MULTI_FIELD_WEIGHTS[field_index])
    return gap
...
def _ca_wiki_paragraph_field_sentence(field_index: int, value: int) -> str:
    term = _CA_WIKI_MULTI_FIELD_TERMS[field_index][int(value)]
    if field_index == 0:
        return f"Current status is {term}."
    if field_index == 1:
        return f"Priority: {term}."
    if field_index == 2:
        return f"Region coverage is {term}."
    return f"Owned by team {term}."


def _ca_wiki_paragraph_field_distractor(field_index: int, value: int) -> str:
    term = _CA_WIKI_MULTI_FIELD_TERMS[field_index][int(value)]
    if field_index == 0:
        return f"Historical status was {term}."
    if field_index == 1:
        return f"Previous priority was {term}."
    if field_index == 2:
        return f"Earlier region was {term}."
    return f"Former owner was team {term}."


def _ca_wiki_paragraph_text(
    claim: int,
    source: int,
    values: np.ndarray,
    revision: int,
    rng: np.random.Generator,
    is_source: bool,
    core_omit_rate: float,
    weak_omit_rate: float,
    distractor_rate: float,
) -> str:
    sentences = []
    for field_index, terms in enumerate(_CA_WIKI_MULTI_FIELD_TERMS):
        omit_rate = core_omit_rate if field_index < 2 else weak_omit_rate
        if rng.random() >= omit_rate:
            if rng.random() < distractor_rate:
                ...
                sentences.append(
                    _ca_wiki_paragraph_field_distractor(
                        field_index,
                        int(rng.choice(alternatives)),
                    )
                )
            sentences.append(_ca_wiki_paragraph_field_sentence(field_index, int(values[field_index])))
    rng.shuffle(sentences)
    prefix = f"claim-{claim:04d} source-{source:02d}." if is_source else f"- claim-{claim:04d}."
    return prefix + " " + " ".join(sentences) + f" Revision {revision}."


def _ca_wiki_parse_paragraph_text(text: str) -> np.ndarray:
    lowered = text.lower()
    patterns = (
        r"current status is (\w+)",
        r"priority:\s*(\w+)",
        r"region coverage is (\w+)",
        r"owned by team (\w+)",
    )
    values = []
    for field_index, pattern in enumerate(patterns):
        parsed = -1
        match = re.search(pattern, lowered)
        if match:
            term = match.group(1)
            if term in _CA_WIKI_MULTI_FIELD_TERMS[field_index]:
                parsed = _CA_WIKI_MULTI_FIELD_TERMS[field_index].index(term)
        values.append(parsed)
    return np.array(values, dtype=np.int64)


def _ca_wiki_parse_paragraph_text_noisy(
    text: str,
    rng: np.random.Generator,
    parser_misread_rate: float,
    parser_drop_rate: float,
) -> Tuple[np.ndarray, int]:
    clean_values = _ca_wiki_parse_paragraph_text(text)
    observed = clean_values.copy()
    miss_count = 0
    for field_index, terms in enumerate(_CA_WIKI_MULTI_FIELD_TERMS):
        if clean_values[field_index] < 0 or rng.random() < parser_drop_rate:
...
def _ca_wiki_multifield_majority_with_fallback(
    values: np.ndarray,
    fallback: np.ndarray,
) -> np.ndarray:
    majority_values = []
    for field_index, terms in enumerate(_CA_WIKI_MULTI_FIELD_TERMS):
        present = values[:, field_index][values[:, field_index] >= 0]
        if len(present) == 0:
            majority_values.append(int(fallback[field_index]))
        else:
            majority_values.append(
                int(np.argmax(np.bincount(present, minlength=len(terms))))
            )
    return np.array(majority_values, dtype=np.int64)
...
def _ca_wiki_text_compiled_trace_labels(
    features: np.ndarray,
    seed: int,
    query_events: int,
    update_events: int,
    ...
    """Derive importance from source-text edits and compiled markdown summaries."""
    ...
    truth_value = rng.integers(0, len(_CA_WIKI_STATUS_TERMS), size=claim_count, dtype=np.int64)
    ...
    for claim in range(claim_count):
        initial_value = int(truth_value[claim])
        for source in range(sources_per_claim):
            source_text[claim, source] = _ca_wiki_source_status_text(
                claim,
                source,
                initial_value,
                0,
            )
        summary_text[claim] = _ca_wiki_summary_status_text(claim, initial_value, 0)
    ...
    rng.shuffle(events)
    probe_count = min(audit_probes, sources_per_claim)
    status_count = len(_CA_WIKI_STATUS_TERMS)
    for event in events:
        if event == 1:
            claim = int(rng.choice(claim_count, p=update_weights))
            update_count[claim] += 1
            dirty_count[claim] += 1
            if rng.random() < (0.35 + 0.25 * recency[claim] / 3.0):
                truth_value[claim] = int((truth_value[claim] + rng.integers(1, status_count)) % status_count)
                claim_revision[claim] += 1
            fanout = int(rng.integers(1, min(4, sources_per_claim) + 1))
            sources = rng.choice(sources_per_claim, size=fanout, replace=False)
            for source in sources:
                if rng.random() < (0.12 + 0.10 * (3.0 - trust[claim]) / 3.0):
                    alternatives = [value for value in range(status_count) if value != truth_value[claim]]
                    value = int(rng.choice(alternatives))
                else:
                    value = int(truth_value[claim])
            ...
            claim = int(rng.choice(claim_count, p=compile_weights))
            values = np.array(
                [_ca_wiki_parse_status_text(text) for text in source_text[claim]],
                dtype=np.int64,
            )
            summary_value = _ca_wiki_majority_value(values)
            summary_text[claim] = _ca_wiki_summary_status_text(
                claim,
                summary_value,
            ...
            claim = int(rng.choice(claim_count, p=query_weights))
            query_count[claim] += 1
            summary_value = _ca_wiki_parse_status_text(str(summary_text[claim]))
            if summary_value != truth_value[claim]:
                retrieval_error_count[claim] += 1
            sources = rng.choice(sources_per_claim, size=probe_count, replace=False)
            probed_values = np.array(
                [_ca_wiki_parse_status_text(str(source_text[claim, source])) for source in sources],
                dtype=np.int64,
            )
            stale_probe_count[claim] += int(np.sum(probed_values != summary_value))
            if np.any(probed_values != truth_value[claim]) or len(set(int(value) for value in probed_values)) > 1:
                contradiction_probe_count[claim] += 1
...
def _ca_wiki_text_noisy_parser_trace_labels(
    features: np.ndarray,
    seed: int,
    query_events: int,
    update_events: int,
    ...
    """Text trace where the CA controller observes noisy parser counters."""
    ...
    status_count = len(_CA_WIKI_STATUS_TERMS)
    truth_value = rng.integers(0, status_count, size=claim_count, dtype=np.int64)
    claim_revision = np.zeros(claim_count, dtype=np.int64)
    source_revision = np.zeros((claim_count, sources_per_claim), dtype=np.int64)
    source_text = np.empty((claim_count, sources_per_claim), dtype=object)
    summary_text = np.empty(claim_count, dtype=object)
    ...
    compile_count = np.zeros(claim_count, dtype=np.int64)
    text_edit_count = np.zeros(claim_count, dtype=np.int64)
    clean_retrieval_error_count = np.zeros(claim_count, dtype=np.int64)
    clean_stale_probe_count = np.zeros(claim_count, dtype=np.int64)
    clean_contradiction_probe_count = np.zeros(claim_count, dtype=np.int64)
    observed_retrieval_error_count = np.zeros(claim_count, dtype=np.int64)
    observed_stale_probe_count = np.zeros(claim_count, dtype=np.int64)
    observed_contradiction_probe_count = np.zeros(claim_count, dtype=np.int64)
    ...
    for event in events:
        if event == 1:
            ...
            claim = int(rng.choice(claim_count, p=query_weights))
            query_count[claim] += 1
            clean_summary_value = _ca_wiki_parse_status_text(str(summary_text[claim]))
            if clean_summary_value != truth_value[claim]:
                clean_retrieval_error_count[claim] += 1
            sources = rng.choice(sources_per_claim, size=probe_count, replace=False)
            clean_values = np.array(
                [_ca_wiki_parse_status_text(str(source_text[claim, source])) for source in sources],
                dtype=np.int64,
            )
            clean_stale_probe_count[claim] += int(np.sum(clean_values != clean_summary_value))
            if (
                np.any(clean_values != truth_value[claim])
                or len(set(int(value) for value in clean_values)) > 1
            ):
                clean_contradiction_probe_count[claim] += 1

            observed_summary_value, miss = _ca_wiki_parse_status_text_noisy(
            ...
            if observed_summary_value < 0 or observed_summary_value != truth_value[claim]:
                observed_retrieval_error_count[claim] += 1
            observed_values = []
            for source in sources:
                observed_value, miss = _ca_wiki_parse_status_text_noisy(
            ...
            observed_stale_probe_count[claim] += int(
                np.sum((observed_values_array < 0) | (observed_values_array != observed_summary_value))
            )
            if (
                np.any((observed_values_array < 0) | (observed_values_array != truth_value[claim]))
                or len(set(int(value) for value in observed_values_array)) > 1
            ):
                observed_contradiction_probe_count[claim] += 1

    risk = (
        3.0 * clean_retrieval_error_count.astype(np.float64)
        + 2.0 * clean_contradiction_probe_count.astype(np.float64)
        + 1.5 * clean_stale_probe_count.astype(np.float64)
        + 0.5 * update_count.astype(np.float64)
        + 1.5 * trust
        + 0.75 * citations
    )
    ...
    return (
        ...
        compile_count,
        text_edit_count,
        clean_retrieval_error_count,
        clean_contradiction_probe_count,
        clean_stale_probe_count,
...
def _ca_wiki_paragraph_text_trace_labels(
    features: np.ndarray,
    seed: int,
    query_events: int,
    update_events: int,
    ...
    for claim in range(claim_count):
        for source in range(sources_per_claim):
            source_text[claim, source] = _ca_wiki_paragraph_text(
                claim,
                source,
                truth_values[claim],
                0,
        ...
        summary_text[claim] = _ca_wiki_paragraph_text(
            claim,
            0,
            truth_values[claim],
            0,
    ...
    observed_summary_core_gap = np.zeros(claim_count, dtype=np.float64)
    observed_source_core_gap = np.zeros(claim_count, dtype=np.float64)
    observed_agreement_gap = np.zeros(claim_count, dtype=np.float64)
    ...
    for event in events:
        if event == 1:
            ...
            compile_weights /= float(np.sum(compile_weights))
            claim = int(rng.choice(claim_count, p=compile_weights))
            old_summary = _ca_wiki_parse_paragraph_text(str(summary_text[claim]))
            fallback = np.where(old_summary >= 0, old_summary, truth_values[claim])
            values = np.array(
                [_ca_wiki_parse_paragraph_text(str(text)) for text in source_text[claim]],
                dtype=np.int64,
            )
            summary_values = _ca_wiki_multifield_majority_with_fallback(values, fallback)
            summary_text[claim] = _ca_wiki_paragraph_text(
                claim,
                0,
                summary_values,
            ...
            claim = int(rng.choice(claim_count, p=query_weights))
            query_count[claim] += 1
            clean_summary = _ca_wiki_parse_paragraph_text(str(summary_text[claim]))
            clean_weighted_error[claim] += _ca_wiki_multifield_weighted_diff(
                clean_summary,
                truth_values[claim],
            )
            ...
            clean_values = np.array(
                [
                    _ca_wiki_parse_paragraph_text(str(source_text[claim, source]))
                    for source in sources
                ],
                dtype=np.int64,
            )
            ...
            observed_summary, miss = _ca_wiki_parse_paragraph_text_noisy(
                str(summary_text[claim]),
                rng,
                parser_misread_rate,
                parser_drop_rate,
            ...
            observed_summary_core_gap[claim] += _ca_wiki_multifield_weighted_missing(
                observed_summary[:2]
            )
            observed_weighted_error[claim] += _ca_wiki_multifield_weighted_diff(
                observed_summary,
            ...
            observed_values = []
            for source in sources:
                observed_value, miss = _ca_wiki_parse_paragraph_text_noisy(
                    str(source_text[claim, source]),
                    rng,
                    parser_misread_rate,
                    parser_drop_rate,
                ...
                observed_source_core_gap[claim] += _ca_wiki_multifield_weighted_missing(
                    observed_value[:2]
                )
                observed_values.append(observed_value)
            observed_values_array = np.array(observed_values, dtype=np.int64)
            observed_agreement_gap[claim] += _ca_wiki_multifield_core_agreement_gap(
                observed_summary,
                observed_values_array,
            )
            observed_weighted_stale[claim] += sum(
    ...
    return (
        ...
        observed_summary_core_gap,
        observed_source_core_gap,
        observed_agreement_gap,
    )
...
def _ca_wiki_metadata_key_teacher(
    ...
    features = np.array(
        [[trust_bucket, citation_bucket, recency_bucket, query_bucket]],
        dtype=np.int64,
    )
    return int(_ca_wiki_metadata_teacher(features)[0])


def run_ca_wiki_cell_metadata_importance_sweep(
    *,
    claim_count: int = 512,
    train_claim_count: int = 4096,
    train_seeds: Tuple[int, ...] = (3001, 3101),
    eval_seeds: Tuple[int, ...] = (3201, 3301, 3401),
    min_accuracy: float = 0.90,
    min_strict_recall: float = 0.90,
    max_under_strict_rate: float = 0.05,
) -> CAWikiCellMetadataImportanceResult:
    """Learn low-bit wiki metadata -> importance mode for provenance repair."""
    ...
    bucket_count = 4
    counts = np.zeros((bucket_count, bucket_count, bucket_count, bucket_count, mode_count), dtype=np.int64)
    for seed in clean_train_seeds:
        features = _ca_wiki_metadata_features(train_claim_count, seed)
        labels = _ca_wiki_metadata_teacher(features)
        ...
        for citation_bucket in range(bucket_count):
            for recency_bucket in range(bucket_count):
                for query_bucket in range(bucket_count):
                    ...
                    if training_count:
                        chosen_index = int(np.argmax(bucket_counts))
                    else:
    ...
    eval_points: List[CAWikiCellMetadataImportancePoint] = []
    for seed in clean_eval_seeds:
        features = _ca_wiki_metadata_features(claim_count, seed)
        teacher = _ca_wiki_metadata_teacher(features)
        predicted = np.array(
            [
        ...
        estimated_touch = sum(mode_rates[index] * mode_touch[modes[index]] for index in range(mode_count))
        accuracy = correct / float(claim_count)
        under_strict_rate = under_strict / float(claim_count)
        eval_points.append(
            CAWikiCellMetadataImportancePoint(
                eval_seed=seed,
                claim_count=claim_count,
                accuracy=accuracy,
                ...
                estimated_policy_lut_bytes=classifier_lut_bytes + provenance.lut_state_bytes,
                target_met=(
    ...
    return CAWikiCellMetadataImportanceResult(
        claim_count=claim_count,
...
def run_ca_wiki_cell_text_trace_importance_sweep(
    *,
    claim_count: int = 1024,
    train_claim_count: int = 4096,
    query_events: int = 4096,
    update_events: int = 2048,
    compile_events: int = 512,
    train_seeds: Tuple[int, ...] = (4301, 4401, 4501),
    eval_seeds: Tuple[int, ...] = (4601, 4701, 4801, 4901),
    under_importance_weight: float = 4.0,
    over_importance_weight: float = 0.75,
    min_accuracy: float = 0.86,
    min_strict_recall: float = 0.99,
    max_under_strict_rate: float = 0.06,
) -> CAWikiCellTextTraceImportanceResult:
    """Learn importance from text-source compiled-wiki error counters."""
    ...
    for seed in clean_train_seeds:
        ...
        (
            ...
            contradiction_probes,
            stale_probes,
        ) = _ca_wiki_text_compiled_trace_labels(
            features,
            seed + 59,
            train_query_events,
            train_update_events,
    ...
    classifier_bits = bucket_count**3 * max(1, int(np.ceil(np.log2(mode_count))))
    classifier_lut_bytes = classifier_bits / 8.0
    eval_points: List[CAWikiCellTextTraceImportancePoint] = []
    for seed in clean_eval_seeds:
        features = _ca_wiki_metadata_features(claim_count, seed)
        (
            teacher,
        ...
        under_strict_rate = under_strict / float(claim_count)
        eval_points.append(
            CAWikiCellTextTraceImportancePoint(
                eval_seed=seed,
                claim_count=claim_count,
                query_events=query_events,
                update_events=update_events,
                ...
                mean_updates_per_claim=float(np.mean(update_count)),
                mean_compiles_per_claim=float(np.mean(compiled_count)),
                mean_source_text_edits_per_claim=float(np.mean(text_edit_count)),
                mean_retrieval_errors_per_claim=float(np.mean(retrieval_errors)),
                mean_contradiction_probes_per_claim=float(np.mean(contradiction_probes)),
                mean_stale_probes_per_claim=float(np.mean(stale_probes)),
                estimated_touch_per_event=estimated_touch,
    ...
    return CAWikiCellTextTraceImportanceResult(
        claim_count=claim_count,
        train_claim_count=train_claim_count,
        query_events=query_events,
        update_events=update_events,
        compile_events=compile_events,
        local_signal_bits_per_claim=6,
        status_token_count=len(_CA_WIKI_STATUS_TERMS),
        classifier_lut_bytes=classifier_lut_bytes,
        repair_lut_bytes=provenance.lut_state_bytes,
        under_importance_weight=under_importance_weight,
        over_importance_weight=over_importance_weight,
...
def run_ca_wiki_cell_text_trace_noise_sweep(
    *,
    claim_count: int = 1024,
    train_claim_count: int = 4096,
    query_events: int = 4096,
    ...
    """Audit text-source importance when the observed parser is noisy."""
    ...
    for seed in clean_train_seeds:
        ...
        (
            ...
            _,
            _,
            observed_retrieval_errors,
            observed_contradiction_probes,
            observed_stale_probes,
            _,
        ) = _ca_wiki_text_noisy_parser_trace_labels(
            features,
            seed + 71,
            train_query_events,
            train_update_events,
        ...
        error_bucket = _ca_wiki_count_bucket(observed_retrieval_errors)
        contradiction_bucket = _ca_wiki_count_bucket(observed_contradiction_probes)
        stale_bucket = _ca_wiki_count_bucket(observed_stale_probes)
        for buckets, label in zip(
            zip(error_bucket, contradiction_bucket, stale_bucket),
            labels,
        ):
    ...
    classifier_bits = bucket_count**3 * max(1, int(np.ceil(np.log2(mode_count))))
    classifier_lut_bytes = classifier_bits / 8.0
    eval_points: List[CAWikiCellTextTraceNoisePoint] = []
    for seed in clean_eval_seeds:
        features = _ca_wiki_metadata_features(claim_count, seed)
        (
            teacher,
        ...
        eval_points.append(
            CAWikiCellTextTraceNoisePoint(
                ...
                strict_rate=mode_rates[2],
                mean_source_text_edits_per_claim=float(np.mean(text_edit_count)),
                mean_clean_retrieval_errors_per_claim=float(np.mean(clean_retrieval_errors)),
                mean_observed_retrieval_errors_per_claim=float(np.mean(observed_retrieval_errors)),
                mean_parser_misses_per_claim=float(np.mean(parser_misses)),
                estimated_touch_per_event=estimated_touch,
                target_met=(
                    accuracy >= min_accuracy
                    and strict_recall >= min_strict_recall
    ...
    return CAWikiCellTextTraceNoiseResult(
        claim_count=claim_count,
        train_claim_count=train_claim_count,
        query_events=query_events,
        update_events=update_events,
...
def run_ca_wiki_cell_paragraph_trace_sweep(
    *,
    claim_count: int = 1024,
    train_claim_count: int = 4096,
    query_events: int = 4096,
...
) -> CAWikiCellParagraphTraceResult:
    """Learn importance from paragraph-style source snippets and summaries."""
    ...
    classifier_bits = bucket_count**4 * max(1, int(np.ceil(np.log2(mode_count))))
    classifier_lut_bytes = classifier_bits / 8.0
    eval_points: List[CAWikiCellParagraphTracePoint] = []
    for seed in clean_eval_seeds:
        features = _ca_wiki_metadata_features(claim_count, seed)
        trace = _ca_wiki_paragraph_text_trace_labels(
            features,
    ...
    return CAWikiCellParagraphTraceResult(
        claim_count=claim_count,
        train_claim_count=train_claim_count,
        query_events=query_events,
        update_events=update_events,
...
def run_ca_wiki_cell_paragraph_split_confidence_sweep(
    *,
    claim_count: int = 1024,
    train_claim_count: int = 4096,
    query_events: int = 4096,
    ...
    regime_counter_under_importance_weight: float = 10.0,
    regime_counter_over_importance_weight: float = 1.0,
    regime_counter_target_penalty_weight: float = 4.0,
    regime_counter_repair_bias_weight: float = 0.04,
    regime_counter_profiles: Tuple[str, ...] = (
        "default",
        "parser_x2",
        "omit_x2",
        "distractor_x2",
    ...
    """Compare split summary/source/agreement confidence controllers."""
    ...
    if coverage_lut_under_importance_weight < 0.0:
        raise ValueError("coverage_lut_under_importance_weight must be non-negative")
    if split_lut_under_importance_weight < 0.0:
        raise ValueError("split_lut_under_importance_weight must be non-negative")
    if split_guard_min_count < 0:
    ...
    if regime_counter_under_importance_weight < 0.0:
        raise ValueError("regime_counter_under_importance_weight must be non-negative")
    if regime_counter_over_importance_weight < 0.0:
        raise ValueError("regime_counter_over_importance_weight must be non-negative")
    if regime_counter_target_penalty_weight < 0.0:
    ...
    clean_regime_profiles = tuple(
        dict.fromkeys(str(value) for value in regime_counter_profiles)
    )
    if len(clean_regime_profiles) == 0:
        raise ValueError("regime_counter_profiles must not be empty")
    allowed_regime_profiles = {
        "default",
        "parser_x2",
        "omit_x2",
        "distractor_x2",
    ...
    if unknown_regime_profiles:
        raise ValueError(f"unknown regime_counter_profiles: {unknown_regime_profiles}")
    ...
    def choose_mode(
        ...
        *,
        ...
        if training_count:
            return int(np.argmin(losses))
        fallback_score = 2 * buckets[0] + buckets[1] + buckets[2]
        if use_coverage_fallback and len(buckets) >= 5:
            fallback_score += buckets[4]
        return 2 if fallback_score >= 4 else 1 if fallback_score >= 2 else 0

    lut4 = np.zeros((bucket_count, bucket_count, bucket_count, bucket_count), dtype=np.int64)
    for index in np.ndindex(lut4.shape):
        lut4[index] = choose_mode(
            counts4[index],
            index,
            under_importance_weight,
    ...
    downgrade_guard = np.zeros(lut7.shape, dtype=np.bool_)
    for index in np.ndindex(lut7.shape):
        bucket_counts = counts7[index]
        lut7[index] = choose_mode(
            bucket_counts,
            index,
            split_lut_under_importance_weight,
            use_coverage_fallback=False,
        )
        total = int(np.sum(bucket_counts))
        strict_fraction = bucket_counts[2] / float(total) if total else 1.0
        downgrade_guard[index] = (
            total >= split_guard_min_count
    ...
    def make_selector_bucket_rows(trace: Tuple[np.ndarray, ...]) -> np.ndarray:
        return np.stack(
            (
    ...
    for profile in clean_selector_profiles:
        for seed in clean_train_seeds:
            ...
            selector_buckets = make_selector_bucket_rows(trace)
    ...
    def predict_regime_candidates(
        ...
        vote_counts = np.zeros(bucket_rows.shape[0], dtype=np.int64)
        for projection, factor_guard in zip(selector_projections, selector_guards):
            vote_counts += factor_guard[
                tuple(bucket_rows[:, index] for index in projection)
            ].astype(np.int64)
        ...
        repair_mask = (
            ...
            & (factor_modes == 1)
            & (core_gap_buckets >= two_branch_coverage_repair_min_core_gap_bucket)
            & (parser_miss_buckets <= two_branch_coverage_repair_max_parser_miss_bucket)
        )
    ...
    def regime_feature_index(
        ...
        parser_high = float(np.mean(bucket_rows[:, 3] >= 2))
    ...
    def regime_branch_loss(labels: np.ndarray, predicted: np.ndarray) -> float:
        ...
        loss = (
            incorrect
            + regime_counter_under_importance_weight * under_strict
            + regime_counter_over_importance_weight * over_strict
        )
        if accuracy < min_accuracy:
            loss += (
                regime_counter_target_penalty_weight
                * claim_total
    ...
    for (
        ...
        for seed in clean_train_seeds:
            ...
            regime_counter_losses[feature_index + (1,)] += regime_branch_loss(
                trace[0],
                repair_modes,
    ...
    def make_point(
        ...
        accuracy = correct / float(claim_count)
        under_strict_rate = under_strict / float(claim_count)
        return CAWikiCellParagraphSplitConfidencePoint(
            variant=variant,
            eval_seed=seed,
            classifier_lut_bytes=classifier_lut_bytes,
            guard_lut_bytes=guard_lut_bytes,
            ...
            normal_rate=mode_rates[1],
            strict_rate=mode_rates[2],
            mean_summary_core_gap_per_claim=float(np.mean(trace[13])),
            mean_source_core_gap_per_claim=float(np.mean(trace[14])),
            mean_agreement_gap_per_claim=float(np.mean(trace[15])),
            estimated_touch_per_event=estimated_touch,
            downgraded_strict_claims=downgraded,
            target_met=(
                accuracy >= min_accuracy
    ...
    classifier4_bytes = bucket_count**4 * max(1, int(np.ceil(np.log2(mode_count)))) / 8.0
    classifier5_bytes = bucket_count**5 * max(1, int(np.ceil(np.log2(mode_count)))) / 8.0
    classifier7_bytes = bucket_count**7 * max(1, int(np.ceil(np.log2(mode_count)))) / 8.0
    guard7_bytes = bucket_count**7 / 8.0
    eval_points: List[CAWikiCellParagraphSplitConfidencePoint] = []
    for seed in clean_eval_seeds:
        features = _ca_wiki_metadata_features(claim_count, seed)
        trace = _ca_wiki_paragraph_text_trace_labels(
            features,
        ...
        eval_points.append(
            make_point(
                ...
                0.0,
                10,
                int(np.sum((base_predicted == 2) & (coverage_predicted < 2))),
            )
        )
        split_guard_predicted = base_predicted.copy()
        downgrade_mask = np.array(
            [
                split_guard_predicted[index] == 2
                and downgrade_guard[
                    int(weighted_error_bucket[index]),
                    int(core_conflict_bucket[index]),
                    int(weighted_stale_bucket[index]),
                    int(parser_miss_bucket[index]),
                    int(summary_core_bucket[index]),
                    int(source_core_bucket[index]),
                    int(agreement_bucket[index]),
                ]
                for index in range(claim_count)
            ],
            dtype=np.bool_,
        )
        split_guard_predicted[downgrade_mask] = 1
        eval_points.append(
            make_point(
                "split_guard7d",
                seed,
                teacher,
                split_guard_predicted,
                trace,
                classifier4_bytes,
                guard7_bytes,
                14,
        ...
        split_lut_predicted = np.array(
            [
                ...
                for error_bucket, conflict_bucket, stale_bucket, miss_bucket, summary_bucket, source_bucket, agreement_value in zip(
                    weighted_error_bucket,
                    core_conflict_bucket,
                    weighted_stale_bucket,
                    parser_miss_bucket,
        ...
        eval_points.append(
            make_point(
                ...
                0.0,
                14,
                int(np.sum((base_predicted == 2) & (split_lut_predicted < 2))),
            )
        )

    return CAWikiCellParagraphSplitConfidenceResult(
        claim_count=claim_count,
        train_claim_count=train_claim_count,
        query_events=query_events,
        update_events=update_events,
    ...
    *,
    ...
    for scenario in clean_scenarios:
        ...
        result = run_ca_wiki_cell_paragraph_factorized_guard_sweep(
            ...
            regime_counter_under_importance_weight=(
                regime_counter_under_importance_weight
            ),
            regime_counter_over_importance_weight=(
                regime_counter_over_importance_weight
            ),
            regime_counter_target_penalty_weight=(
                regime_counter_target_penalty_weight
