...
def robust_retiring_sidecar_csa_hca_context_budget() -> ContextSummaryBudget:
    """Joint CSA/HCA budget with the adversarial-robust 3-bit retirement sidecar."""
    ...
    csa_directory_read = 0.50
    csa_token_read = 331.6 * 16 / 8
    sidecar_state = 35.9 * 1024
    sidecar_read = 3 / 8
    sidecar_update = 0.223
    return ContextSummaryBudget(
        block_summary_state_bytes=block_summary_state,
        csa_directory_state_bytes=directory_state,
        control_lut_state_bytes=fanout_lut_state + probe_lut_state + spread_metadata_state,
...
def fanout_guard_retiring_sidecar_csa_hca_context_budget() -> ContextSummaryBudget:
    """Robust sidecar budget with the selective zero-overlap fanout guard.

    The guard floors rare-directory reads at three only when CSA-selected blocks
    overlap none of the exact rare-directory entries. The normal reference path
    still skips directory entries through the HCA/directory control plane, so
    the unified event-profile state and reference traffic remain equal to
    ``retire128c3``.
    """
