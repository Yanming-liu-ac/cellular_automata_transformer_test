"""Paragraph-style text-source CA Wiki Cell importance audit."""
...
from cellular_transformer.wiki_memory import (
    ...
    run_ca_wiki_cell_paragraph_trace_sweep,
)
...
def print_result(result: CAWikiCellParagraphTraceResult) -> None:
    ...
    print("CA Wiki Cell paragraph text trace audit")
    print(
    ...
    print("- Source cells are paragraph snippets, not fixed key-value strings.")
    print("- Historical distractor sentences and omitted fields make parser confidence part of the CA signal.")
    print("- The CA-facing controller remains four 2-bit buckets and a 64B classifier LUT.")
    print("- High over-strict rate is the expected cost of preserving strict recall under paragraph noise.")
    print(f"- Target failures: {failures}/{len(result.points)} evaluation rows.")


def main() -> None:
    print_result(run_ca_wiki_cell_paragraph_trace_sweep())
