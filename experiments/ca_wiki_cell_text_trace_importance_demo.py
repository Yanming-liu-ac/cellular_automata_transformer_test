"""Text-source compiled-wiki trace importance audit for CA Wiki Cell repair."""
...
from cellular_transformer.wiki_memory import (
    ...
    run_ca_wiki_cell_text_trace_importance_sweep,
)
...
def print_result(result: CAWikiCellTextTraceImportanceResult) -> None:
    ...
    print("CA Wiki Cell text-source trace importance audit")
    print(
        ...
        f"compile_events={result.compile_events}, status_tokens={result.status_token_count}, "
    ...
    for point in result.points:
        row = [
            ...
            f"{point.mean_source_text_edits_per_claim:0.2f}",
    ...
    print("- Raw source cells are controlled text snippets with parsed status fields.")
    print("- Compiled markdown summaries can lag source edits and create retrieval errors.")
    print("- The CA-facing controller still sees only three 2-bit local audit counters.")
    print(f"- Target failures: {failures}/{len(result.points)} evaluation rows.")


def main() -> None:
    print_result(run_ca_wiki_cell_text_trace_importance_sweep())
