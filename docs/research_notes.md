...
The compiled-trace audit is the next step in that direction. It models raw
source cells, a compiled claim summary, source updates, summary compile events,
and query-time audits. The learned controller sees only three 2-bit local
counters: retrieval errors, contradiction probes, and stale-source probes. A
16B LUT maps those buckets to loose/normal/strict and reuses the 1.125B
provenance repair table. Held-out strict recall is 99.19-100.00%, with
4.00-6.05% under-strict rate and about 8.36-8.40 touched cells/event. This is
still synthetic, but it is no longer just a hand pressure formula: the labels
are generated by failures in a compiled wiki update loop.

The text-source version keeps the same chip-facing control path but swaps the
integer facts for controlled source snippets and compiled markdown summaries.
Source edits rewrite `status=...` text, compilation emits a markdown summary,
and query/audit probes parse the text back into local error counters. The 16B
LUT reaches 99.46-100.00% strict recall and 4.39-5.37% under-strict rate on
held-out traces. This is a useful intermediate target: it tests the
